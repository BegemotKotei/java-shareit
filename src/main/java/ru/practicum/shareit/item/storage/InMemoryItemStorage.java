...
public class InMemoryItemStorage implements ItemStorage {
    ...
    public Item createItem(User user, Item item) {
        ...
        log.info("New item got itemID: {}, owner is userID: {}.", item.getId(), user.getId());
        return item;
    }
    ...
    public Item updateItem(Long itemId, Item item, Long userId) {
        ...
        log.info("ItemID: {} was updated by userID: {}.", itemId, userId);
    ...
    public Item getItem(Long itemId) {
        checkItemInBase(itemId);
        log.info("Item with itemID: {} was founded.", itemId);
        return items.get(itemId);
    }
    ...
    public List<Item> getAvailableItems(Long userId, String text) {
        ...
        if (!text.isBlank()) {
            itemsForUser = items.values().stream()
